...
class ClusterRecommender:
    ...
    k: int = 10,
    dentro_cluster: bool = True,
    return_query_cluster: bool = False
    ) -> pd.DataFrame:
    
        self._ensure_fitted()

        # 1) construir DF de 1 fila con las columnas esperadas por el pipeline
        row = {"__text_all__": str(text)}
        for c in self.cat_cols:
            if c in self._df.columns:  # solo las categóricas que existían al fit
                row[c] = ""            # valor vacío para no influir
        df_q = pd.DataFrame([row])

        # 2) embedding reducido del query (misma TFIDF+SVD entrenada)
        q_red = self._feature_pipeline.transform(df_q)
        ...
        # 4) similitud coseno contra catálogo
        sims = cosine_similarity(q_red, self._X_reduced[cand_idx])[0]

        out = self._df.iloc[cand_idx].copy()
        out["__sim__"] = sims
        if return_query_cluster and cl is not None:
            out["query_cluster"] = cl

        cols = [
            "article_id", "product_code", "prod_name",
            "garment_group_name", "section_name", "index_name",
            "perceived_colour_master_name", "graphical_appearance_name",
            "__text_all__", "__sim__"
        ]
        cols = [c for c in cols if c in out.columns]
