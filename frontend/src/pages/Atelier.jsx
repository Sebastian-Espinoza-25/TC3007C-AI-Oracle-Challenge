...
const Atelier = () => {
  ...
  const handleSend = async (e) => {
    ...
    const botTyping = {
      sender: "Atelier",
      text: (
        <div className="text-gray-600">Atelier está escribiendo...</div>
      ),
      timestamp: getTimestamp(),
    };
    ...
    setCurrentConv((prev) => ({
      ...prev,
      messages: [...prev.messages, botTyping],
    }));
    ...
    try {
      ...
      const resp = await fetch(`${import.meta.env.VITE_API_URL}/ideator/chat`, {
        method: "POST",
        body: formData,
      });
      ...
      if (!data.recommendations || data.recommendations.length === 0) {
        const botFinal = {
          sender: "Atelier",
          text: data.answer || "Puedo ayudarte con moda y estilo.",
          timestamp: getTimestamp(),
        };

        setCurrentConv((prev) => ({
          ...prev,
          messages: [...prev.messages.slice(0, -1), botFinal],
        }));
      ...
      const items = await Promise.all(
        data.recommendations.map(async (id) => {
          const r = await fetch(`${import.meta.env.VITE_API_URL}/catalog/${id}`);
          if (!r.ok) return null;
          return await r.json();
        })
      );

      const validProducts = items.filter((x) => x);
        ...
        text: (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {validProducts.map((p) => (
                <div
                  key={p.external_article_id}
                  className="bg-white rounded-xl shadow-md p-4 border border-gray-200"
                >
                  <img
                    src={p.image_url}
                    className="w-32 h-48 rounded-lg mx-auto mb-3"
                  />
                  <h3 className="text-sm font-semibold text-center">
                    {p.prod_name}
                  </h3>
                  <p className="text-primary-500 text-center font-bold">
                    ${p.price}
                  </p>

                  <button
                    onClick={() => handleAddToCartAtelier(p)}
                    className="mt-2 bg-primary-500 text-white px-3 py-1 rounded-lg w-full"
                  >
                    Añadir al carrito
                  </button>
                </div>
      ...
      const botError = {
        sender: "Atelier",
        text: "No pude procesar tu solicitud. Intenta nuevamente.",
        timestamp: getTimestamp(),
      };

      setCurrentConv((prev) => ({
        ...prev,
        messages: [...prev.messages.slice(0, -1), botError],
      }));
    }
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    // User bubble: image + optional text
    const userMsg = {
      sender: "Usuario",
      text: (
    ...
    const botTyping = {
      sender: "Atelier",
      text: <div className="text-gray-600">Atelier está escribiendo...</div>,
      timestamp: getTimestamp(),
    };

    let updatedConv = {
      ...currentConv,
      messages: [...currentConv.messages, userMsg, botTyping],
    };
    ...
    formData.append(
      "text",
      userText || "Analiza esta imagen y dame recomendaciones similares"
    );
    ...
    try {
      ...
      if (!data.recommendations || data.recommendations.length === 0) {
        const botFinal = {
          sender: "Atelier",
          text: data.answer || "No encontré recomendaciones para esta imagen.",
          timestamp: getTimestamp(),
        };

        updatedConv = {
          ...updatedConv,
          messages: [...updatedConv.messages.slice(0, -1), botFinal],
        };
        setCurrentConv(updatedConv);
        return;
      }
      ...
      const validProducts = recs.filter((p) => p !== null);
      ...
      const botFinal = {
        ...
        text: (
          <div className="space-y-4">
            <p>
              {data.answer ||
                "He analizado tu imagen. Aquí tienes algunas recomendaciones basadas en tu estilo:"}
            </p>
              ...
              {validProducts.map((p) => (
                ...
                >
                  <img
                    src={p.image_url}
                    alt={p.prod_name}
                    className="rounded-lg mb-3 w-32 h-48 object-cover mx-auto"
                  />
                  ...
                  <button
                    onClick={() => handleAddToCartAtelier(p)}
                    className="mt-3 bg-primary-500 text-white px-3 py-1 rounded-lg w-full"
                  >
                    Añadir al carrito
                  </button>
                </div>
      ...
      const botError = {
        sender: "Atelier",
        text: "No se puede analizar la imagen. Intenta de nuevo.",
        timestamp: getTimestamp(),
      };

      updatedConv = {
        ...updatedConv,
        messages: [...updatedConv.messages.slice(0, -1), botError],
      };
  ...
  //Add to cart for the cards in Atelier

  const handleAddToCartAtelier = (product) => {
    if (!isLoggedIn) {
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col h-full">
              ...
              <div className="flex flex-col items-center text-gray-700 mt-20 mb-20">
                ...
                >
                  <div
                    ...
                    className="flex items-start gap-4 bg-white border border-neutral-300 hover:border-primary-500 transition rounded-2xl p-6 shadow-md cursor-pointer"
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Dime qué ocasión tienes y te recomiendo el look ideal.
                      </p>
                    </div>
                  </div>
          ...
          >
            ...
            <button
              type="submit"
              className="text-2xl text-primary-500 hover:text-primary-400 transition ml-3"
            >
              ➤
            </button>
          </form>
